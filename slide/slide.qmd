...
::: {.mermaid-big}
```{mermaid}
%%{init: {"themeVariables": {"fontSize": "28px"}, "flowchart": {"useMaxWidth": false, "nodeSpacing": 40, "rankSpacing": 50, "padding": 16}}}%%
flowchart TB
  SRC["Julia source"] --> CST["parse → CST"]
  CST --> IR["lower → Core IR"]
  IR --> BC["compile → bytecode"]
  BC --> OUT["stack VM interprets → result"]
```
:::
